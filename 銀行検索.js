...
ddl.addEventListener('change',function(){
        ...
        let tableresult = reader.result.split('\r\n');
        let td = reader.result.split(/\r\n|,/);
        table.innerHTML = `<tr style="background-color: yellow;"><th>金融機関コード</th><th>支店コード</th><th>支店名カナ</th><th>支店名</th></tr>`
         for(let i = 0; i <= tableresult.length; i++){
            if (tableresult[i] === undefined){ none = "none"}
            else{none = "null"}
            if(td[i*5] == ddl.value){if(td[i*5+4] == 2){
         table.innerHTML += `<tr style="display:${none}"><td>${td[i*5]}</td><td>${td[i*5+1]}</td><td>${td[i*5+2]}</td><td>${td[i*5+3]}</td></tr>`;}    }}})
...
searchbtn.addEventListener('click',function(){
      ...
      table.innerHTML = `<tr style="background-color: yellow;"><th>金融機関コード</th><th>支店コード</th><th>支店名カナ</th><th>支店名</th></tr>`
            ...
            for(let i = 0; i <= tableresult.length; i++){
              if(td[i*5] == ddl.value){
              if (td[i*5+3].indexOf(search.value) !== -1){ table.innerHTML += `<tr><td>${td[i*5]}</td><td>${td[i*5+1]}</td><td>${td[i*5+2]}</td><td>${td[i*5+3]}</td></tr>`;}
          }}})})